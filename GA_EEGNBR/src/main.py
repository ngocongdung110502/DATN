...
mutation_rate = int(parser['Genetic Algorithm']['mutation_rate'])

sinks, sources, nodes = initialize_node(int(parser['Sensor']['num_nodes']), int(parser['Sensor']['num_sinks']), int(parser['Sensor']['num_sources']), int(parser['Sensor']['space_dim']), int(parser['Sensor']['depth_range']))
all_nodes = sinks + sources + nodes

for node in all_nodes:
    print(node)

population = initialize_population(all_nodes, sources, sinks, int(parser['Constraints']['radius']), int(parser['Genetic Algorithm']['population_size']))
best_path = genetic_algorithm(population, all_nodes, sinks, sources,
                              int(parser['Genetic Algorithm']['num_generations']), int(parser['Genetic Algorithm']['population_size']),
                              int(parser['Genetic Algorithm']['mutation_rate']), int(parser['Constraints']['radius']))
