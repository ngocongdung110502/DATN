...
class SensorNode:
    def __init__(self, id, position, is_sink=False, is_source=False, bit_rate=10e3, speed_of_sound=1500, initial_energy=30, tx_power=2, rx_power=0.1):
        self.id = id
    ...
    def broadcast_packet(self, packet):
        updated_nodes = []
        self.consume_energy('tx', packet_size=4)

        for neighbor in self.neighbors:
            neighbor.consume_energy('rx', packet_size = 4)
    ...
    # Hàm nhận gói query, khi đó node nhận gói query sẽ gửi lại cho node source gói ack và gửi tiếp gói query đến các node hàng xóm 
    def receive_query_packet(self, query_packet, communication_radius):
        self.consume_energy('rx', packet_size=4)
        ...
        if query_packet.hc > self.hc and query_packet.sender_id not in self.memory_queue:
            ...
            for neighbor in self.neighbors:
                query_packet = QueryPacket(self.id, self.id, self.dpsn, self.hc)
                self.send_query_packet(neighbor, query_packet, communication_radius, timer)
                self.consume_energy('tx', packet_size=4)
            
            return ack_packet
        return None
    
    # Hàm gửi gói query đến các node hàng xóm. Sau dó nhận được các gói ack của các node hàng xóm gửi đến.
...
def simulate_guiding_network(num_nodes, num_sinks, space_dim, max_depth, depth_layers, communication_radius, xy_limit, energy_threshold):
    ...
    for max_hc_node in max_hc_nodes:
        ...
        while not current_node.is_sink:
            ...
            if ack_packets:
                sender_ids = [p.sender_id for p in ack_packets]
                print(f'Node {current_node.id} received ack_packets from sender_ids: {sender_ids}')
            else:
                print(f'Node {current_node.id} did not receive any ack_packets.')
